...
#if BOARD
  uint8_t maxraw_fd = RAW1_FD;
#else 
  uint8_t maxraw_fd = RAW3_FD;
#endif // BOARD

#if RAW
  uint8_t tmpcpt=(*readcpt);
  for (devcpt = RAW0_FD; devcpt <= maxraw_fd; devcpt++) {
    if (wfb_net_init(&dev[devcpt].raw)) {
      if (INITCHAN[(devcpt - RAW0_FD) / maxraw_fd][(devcpt - RAW0_FD) % maxraw_fd] < dev[devcpt].raw.freqsnb) {
        if (wfb_net_setfreq( INITCHAN[(devcpt - RAW0_FD) / maxraw_fd][(devcpt - RAW0_FD) % maxraw_fd] , &dev[devcpt].raw )) {
          printf("DEV [%s][%d]\n", dev[devcpt].raw.name, INITCHAN[(devcpt - RAW0_FD) / maxraw_fd][(devcpt - RAW0_FD) % maxraw_fd]);
