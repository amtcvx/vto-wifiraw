...
sudo ./exe_synchcliraw $DEVICE1
sudo ./exe_synchcliraw $DEVICE1 $DEVICE2
*/
...
/******************64 bytes from 10.0.1.2: icmp_seq=38 ttl=64 time=3.16 ms
************************************************************/
...
int main(int argc, char **argv) {
  ...
  int16_t probuf[MAXRAWNB]; memset(probuf,0,sizeof(probuf));
...
//                rawdevs[cpt-minraw].synccum++;
              ...
              if (rawdevs[rawcpt].syncchan != headspay.chan) {
                ...
                rawdevs[rawcpt].syncchan = headspay.chan;
