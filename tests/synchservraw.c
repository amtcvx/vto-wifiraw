...
gcc -g -O2 -Wall -Wundef -Wstrict-prototypes -Wno-trigraphs -fno-strict-aliasing -fno-common -Werror-implicit-function-declaration -DRADIOTAPSIZE=$RADIOTAPSIZE --DCONFIG_LIBNL30 -I/usr/include/libnl3 -c synchservraw.c -o synchservraw.o

cc synchservraw.o -g -lnl-route-3 -lnl-genl-3 -lnl-3 -o exe_synchservraw
      ...
      for (uint8_t cpt=0; cpt<readnb; cpt++) {
	    ...
	    for (uint8_t rawcpt = 0; rawcpt < rawnb; rawcpt++) {
	      ...
	      rawdevs[rawcpt].synccum = 0;
            ...
            if (mainraw < 0) {
              for (uint8_t rawcpt = 0; rawcpt < rawnb; rawcpt++)
                if (rawdevs[rawcpt].freefreq) mainraw = rawcpt;
            } else {
              if ((!(rawdevs[mainraw].freefreq)) && (backraw >= 0) && (rawdevs[backraw].freefreq))
	        { mainraw = backraw; backraw = -1; }
	    }

            if (backraw < 0) {
              for (uint8_t rawcpt = 0; rawcpt < rawnb; rawcpt++)
	        if ((rawdevs[rawcpt].freefreq) && (rawcpt != mainraw)) backraw = rawcpt;
	    } else if (!(rawdevs[backraw].freefreq)) backraw = -1;
